...
go_windows_ps <- function(gene, tss_ps){
  ...
  w_start = seq(from = gene_start, to = gene_end, by = 100)
  w_end = seq(from = w_start[2]-1, to = gene_end, by = 100)
...
tss_position = read.csv("C:/Users/lenovo/Desktop/unified_model/data/cd14_cd4_plus_TSS_union.tsv", 
                        sep = '\t', header = T)

cd14_dft = lapply(final_gene_set, go_windows_ps, tss_position) %>% data.table::rbindlist()
...
gr$count <- tuSelecter2::summarize_bigwig(bigwig_file = cd14_bw_plus,
                                       gr,
...
plot_dir = "C:/Users/lenovo/Desktop/unified_model/acf_plot/" 
gr = readRDS(paste0(output_dir, 'cd14_plus_windows_rc.RData'))

sample_genes = final_gene_set[c(1)]
for(gene in sample_genes){
  ...
  diff_sum = c(diff_sum, sum(c[c(i:length(c))]))
  
  # creat df for acf, rolling mean, and sum of fist differences
  acf_df <- data.frame(index = c(1: 200), acf_values)
  rollmean_df <- data.frame(index = c(1:200), rolling = b[c(1:200)])
  diffsum_df <- data.frame(index = c(1:200), first_diff = diff_sum[c(1:200)])
  
  all_df <- cbind(acf_df, rollmean_df, diffsum_df)
  ...
  p = ggplot(data = mydata,aes(x=index,y=value,group = variable,color=variable,shape=variable))+
    ...
    geom_line()+
    xlab("index")+#横坐标名称
    ylab(paste0(gene, "    values"))+#纵坐标名称
    theme_bw() +#去掉背景灰色
    theme(panel.grid.major=element_line(colour=NA),
          panel.background = element_rect(fill = "transparent",colour = NA),
          plot.background = element_rect(fill = "transparent",colour = NA),
          panel.grid.minor = element_blank(),#去除网格线且保留坐标轴边框
          legend.position = c(0.9,0.1))#更改图例的位置，放至图内部的左上角
  #scale_x_continuous(limits = c(2000,2018),breaks = seq(2000,2018,1))#更改横坐标刻度

}

}
p
ggsave(p,filename = paste0(plot_dir, gene, ".pdf"), width = 12,height = 9)
 

################# another t series analysis #####################
ns = c()
for(gene in final_gene_set){
  n = length(gr[gr$gene_id == gene]$count)
  ns = c(ns, n)}
summary(ns) 
15*100/1000

sample_genes = plus_gene_set[c(20)]
  ...
  # creat df for acf, rolling mean, and sum of fist differences
    ...
    p = ggplot(data = mydata,aes(x=index,y=value,group = variable,color=variable,shape=variable))+
      ...
      theme(panel.grid.major=element_line(colour=NA),
            ...
            plot.background = element_rect(fill = "transparent",colour = NA),
            panel.grid.minor = element_blank(),#去除网格线且保留坐标轴边框
            legend.position = c(0.8,0.3))+#更改图例的位置，放至图内部的左上角
      theme(axis.text=element_text(size=10,face = "bold"), #坐标轴刻度值的字体大小
            axis.title.x=element_text(size=10),axis.title.y=element_text(size=10),
             legend.key.size = unit(20, "pt"), legend.text=element_text(size=rel(1)))
    #scale_x_continuous(limits = c(2000,2018),breaks = seq(2000,2018,1))#更改横坐标刻度
    
    # 单独plot acf 
    acf(data, plot = T, lag.max = 40)
