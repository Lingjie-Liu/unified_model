...
#############################################################################
### add cases
data_dir = 'unified_model/data/'
hs_grng =readRDS(paste0(root_dir, data_dir, 'hsapiens_transcript_grng.RDS'))
tss_position = readRDS(paste0(root_dir, data_dir, 'cd14_cd4_TSS_union.Rdata'))
...
output_dir <- paste0(root_dir, 'unified_model/beta_plot/')
pausing_only = readRDS(paste0(root_dir, data_dir, "pausing_only.RData"))
pausing_only = arrange(pausing_only, q_value)
target_genes = as.character(pausing_only$gene_id)
sample_cases = target_genes[c(1:5)]
...
output_dir <- paste0(root_dir, 'unified_model/alpha_plot/')
initiation_only = readRDS(paste0(root_dir, data_dir, "initiation_only.RData"))
initiation_only = arrange(initiation_only, q_value)
target_genes = as.character(initiation_only$gene_id)
sample_cases = target_genes[c(1:5)]
for (i in sample_cases){
  view_cases(i, extend_region, l_length)
}
