...
tss_zeta = rep(1, length.out = 100)

sim_zeta_tbl = simulator_gb %>%
...
see = test %>% filter(ensembl_gene_id == 2)

bf = sim_zeta_tbl$zeta
af = sim_zeta_tbl$zeta

unique(af == bf)


## save data for simulator : full 2000 sites of a cell
sim_data_out = paste0(root_dir, '/simulator/sim_data/sim_zeta.R')
saveRDS(sim_zeta_tbl, sim_data_out)

## save data as gb demo that can input into glm : only 1900 sites (gene bodies) of a cell
sim_gb_out = paste0(root_dir, '/simulator/sim_gb/sim_gb.R')
saveRDS(sim_gb, sim_gb_out)
sampled_cov_out = paste0(root_dir, '/simulator/sim_gb/sim_cov.R')
saveRDS(Yji, sampled_cov_out)

# df = tibble(x = c(1,1,2,2), y = c(2,3,4,5))
# df %>% group_by(x) %>%
#   group_modify(~bind_rows(tibble(y =  rep(1, length.out = 4)), .))
